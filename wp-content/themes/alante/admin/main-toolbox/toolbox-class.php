...
//			    add_menu_page( 'My Page Title', $this->menu_name, 'activate_plugins', 'thinkup-setup', '', get_template_directory_uri() . '/admin/main-toolbox/assets/img/icon.png' );
				...
				add_submenu_page(
					'thinkup-setup',
					esc_html__( 'Guide', 'thinkup-toolbox' ),
					esc_html__( 'Guide', 'thinkup-toolbox' ),
					'activate_plugins',
					'thinkup-setup',
					array(
			    ...
			    // add_submenu_page(
				// 	'thinkup-setup',
				// 	esc_html__( 'Free vs Pro', 'thinkup-toolbox' ),
				// 	esc_html__( 'Free vs Pro', 'thinkup-toolbox' ),
				// 	'activate_plugins',
				// 	'thinkup-pro',
				// 	array(
				...
				add_submenu_page(
					'thinkup-setup',
					esc_html__( 'Upgrade Now', 'thinkup-toolbox' ),
					esc_html__( 'Upgrade Now', 'thinkup-toolbox' ),
					'activate_plugins',
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( $theme_name . '_thinkup_notice_welcome' ) ) {
					...
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'thinkup-toolbox'). '</span>';
					echo '</a>';
		...
		public function hide_notice() {
			if (isset($_GET['thinkup-hide-notice']) && isset($_GET['_thinkup_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_thinkup_notice_nonce'], 'thinkup_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'thinkup-toolbox'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'thinkup-toolbox'));
				}
